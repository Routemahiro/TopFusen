...
## 3. ~~仮想デスクトップと付箋の紐づけが外れるケース~~ ✅ Phase 13 で修正済み

**起票日**: 2026-02-07
**解決日**: 2026-02-07
**関連**: Phase 13（VD 紐づけバグ修正）

### ✅ 解決済み — Phase 13 で以下の3件を修正

1. **BUG 1（主因）**: `DuplicateNote()` で `DesktopId` をコピーしていなかった → `model.DesktopId = source.Model.DesktopId;` を追加
2. **BUG 2**: `HandleDesktopSwitch()` のリアルタイム孤立判定が誤判定 → 削除（起動時の `RescueOrphanedNotes()` のみに限定）
3. **BUG 3**: `CreateNote()` で `GetCurrentDesktopIdFast()` が null 時のフォールバック不足 → 重量級 `GetCurrentDesktopId()` にフォールバック
