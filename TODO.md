...
## Phase 11: 設定画面（統合）⏳ (作業中)
> 目標: 全設定項目を統合した設定ウィンドウ
> 実装方針: **案B（バランス）** — 4タブ構成、ホットキーはプリセット選択方式（自由キー設定は延期）

- [ ] P11-1: 設定ウィンドウ骨格（TabControl + 4タブ XAML）
- [ ] P11-2: 一般タブ — 自動起動ON/OFF、ホットキー設定（ON/OFF + プリセットドロップダウン）
- [ ] P11-3: フォントタブ — 許可リスト管理（追加/削除）
- [ ] P11-4: 付箋管理タブ — Z順一覧（D&D）統合
- [ ] P11-5: 詳細タブ — ログフォルダを開く、診断パッケージ生成（FR-DEBUG）
- [ ] P11-6: App.xaml.cs 連携（トレイメニュー → 設定画面を開く + 設定同期）
- [ ] P11-7: 設定変更の即時反映 + 永続化
